...
async function startServer({ port, lockVars, perMessageDeflate }) {
  ...
  app.listen(port, async () => {
    ...
    console.log(
      `  • on your computer at ${colours.cyan(`ws://localhost:${port}/`)} (use this for testing)`,
    );
    const privateIp = await internalIp.v4().catch(() => null);
    if (privateIp) {
      console.log(
        `  • locally within your network at ${colours.blue(`ws://${privateIp}:${port}/`)} (maybe)`,
      );
    }
    const ip = await publicIp.v4().catch(() => null);
    if (ip) {
      console.log(
        `  • publicly at ${colours.blue(`ws://${ip}:${port}/`)}, but ONLY if you've set up port forwarding on your router`,
      );
    }
    console.log(
      colours.yellow(
        `Plain http requests will be served using files from the static/ folder, which you can access in your browser at ${colours.blue(`http://localhost:${port}/`)}.`,
      ),
    );
