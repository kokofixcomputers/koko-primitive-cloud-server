  ...
  handleWsConnection(ws) {
    ...
    ws.on("message", (data) => {
      ...
      try {
        ...
        console.error(
          "Invalid JSON has been received over the Websocket connection.",
        );
        console.error(data);
        console.error(err);
        console.warn(
          "This might mean that someone is trying to tamper with your server.",
        );
        return;
      }
      switch (message.method) {
          ...
          if (!handshaken) {
            ...
            this.getProject(message.project_id).then((projectData) => {
              if (projectData) {
                ...
                const changes = Object.entries(project.variables).map(
                  ([variable, value]) => ({
                    method: "set",
                    name: variable,
                    value,
        ...
        case "rename":
          if (project && !this.lockVars) {
            project.variables[message.new_name] =
              project.variables[message.name];
        ...
        default:
          console.error(
            `A unknown method has been received over Websocket:${message.method}.`,
          );
      }
    });
